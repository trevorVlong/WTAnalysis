function [corrected2D,uncorrected2D] = nonDimVecs(runfile,calfile,tarefile)
...
%save('filename.mat','-append'); to do
    ...
    load('options.mat','wing_geom','paths');
    
    addpath(paths.runpath,paths.tarepath,paths.calpath) % adjust in "options' file
%     addpath('MotorModel','WallCorrections');
    
    calfile = strcat(paths.calpath,calfile);
    tarefile = strcat(paths.tarepath,tarefile);
    runfile  = strcat(paths.runpath,runfile);
...
%----------------------------------
% important coefficients/aero stuff
    corrected2D = twallvec(uncorrected2D,walldata); %includes cl,cx Vinf_corrected
