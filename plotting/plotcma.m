function [] = plotcma(file,fig,plotops)
...
% ----------------------------------- 
    ...
    ax = scatter(alfa(:),cm2D(:),20, dcj(:),'filled');
    ...
    if plotops.interp == 1
        ...
        for ndcj = 1:5
            ...
            cminterp = Vcm(alfavec,dcjvec(ndcj)*ones(size(alfavec)));
            
            plot(alfavec,cminterp,...
    ...
    xlim(plotops.alpharange);
    ylim([-0.75,0.75]);
    % labels, 
    xlabel('$\alpha$','Interpreter','latex','FontSize',plotops.labelFS);
    ylabel('$c_m$','Interpreter','latex','FontSize',plotops.labelFS);
